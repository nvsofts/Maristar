...
var langResources = {
	...
	'API proxy': ['APIプロキシ',null],
	'memcached server': ['memcachedサーバ',null],
	...
	'Are you sure to logout? You need to re-authenticate Maristar at next launch.': ['ログアウトしてもよろしいですか? 次回起動時には再認証が必要になります。','确定要注销？下次启动的时候需要重新进行认证。'],
	...
	'An insecure external plugin is prohibited. Use HTTPS.': ['安全でない場所からのプラグインのロードはできません。HTTPSを使用してください。', null],
	'An external plugin is specified. This plugin can fully access to your account and Firefox.\nAre you sure to load this?': ['外部プラグインが指定されています。このプラグインはあなたのアカウントとFirefoxに自由にアクセス可能になります。本当にロードしてもよろしいですか?',null],
	'Cannot access to direct messages. Please re-auth Maristar for DM access.': ['ダイレクトメッセージにアクセスできません。お手数ですが再度認証を行ってください。'],
	'Cannot tweet from Maristar? Please try logging out of Twitter web site...': ['もしツイートがうまくできない場合、TwitterのWebサイトからログアウトすると成功することがあります。'],
